...
export function IssueForm({ platforms, preselectedPlatformId }: IssueFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ─────────────────────────────────────────────────────────────
      // Demo Mode Fallback (لتجربة واجهة الإرسال بدون قاعدة بيانات)
      // ─────────────────────────────────────────────────────────────
      if (err?.message?.includes('Failed to fetch') || typeof window !== 'undefined' && document.cookie.includes('demo_admin=true')) {
        setTimeout(() => {
          try {
            const fakeId = Math.random().toString(36).substring(7);
            const newTicket = {
              id: 'demo-' + fakeId,
              ticket_number: 'RAF-DEMO-' + Math.floor(1000 + Math.random() * 9000),
              platform_id: formData.platform_id,
              user_type: formData.user_type,
              wilaya: formData.wilaya,
              description: formData.description,
              ...
              status: 'new',
              solution: null,
              ai_diagnosis: 'جاري تشخيص المشكلة...',
              priority: 'normal',
              created_at: new Date().toISOString(),
              updated_at: new Date().toISOString(),
              resolved_at: null,
              platform: platforms.find(p => p.id === formData.platform_id) || { name: 'منصة غير معروفة' }
            };
            const stored = localStorage.getItem('demo_submitted_tickets');
            const prevTickets = stored ? JSON.parse(stored) : [];
            localStorage.setItem('demo_submitted_tickets', JSON.stringify([newTicket, ...prevTickets]));
            
            addToast('success', `تم إرسال انشغالك بنجاح (وضع التجريب) — رقم التذكرة: ${newTicket.ticket_number}`);
          } catch(e) {}
      ...
      if (!document.cookie.includes('demo_admin=true')) {
        setLoading(false);
      }
    }
  };
